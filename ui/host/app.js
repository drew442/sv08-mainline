...
function capability(action, button, reason) {
    ...
    $(button).disabled = busy || !cap.available;
...
async function refresh() {
    try { state = await request({method: 'status'}); render(); }
    catch (error) { $('connection').textContent = 'Host unavailable'; notice(error.message); document.querySelectorAll('main button').forEach(b => { b.disabled = true; }); }
}
...
$('review').addEventListener('close', async () => {
    ...
    try { const result = await request({method: 'apply', plan}); notice(result.message || 'Change completed.'); }
