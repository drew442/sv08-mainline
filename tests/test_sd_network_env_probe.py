...
LOCATOR = ROOT / 'tests/fixtures/sd-network-root/emmc_locator.h'
BUILDER = ROOT / 'scripts/build_sd_network_image.py'
...
class SdNetworkEnvProbeTests(unittest.TestCase):
    ...
    def test_probe_has_bounded_read_only_eMMC_access(self):
        ...
        locator = LOCATOR.read_text()
        builder = BUILDER.read_text()
        self.assertIn('#define ENV_A_OFFSET 0x400000', source)
        self.assertIn('#define ENV_B_OFFSET 0x800000', source)
        self.assertIn('#include "emmc_locator.h"', source)
        self.assertIn('/4022000.mmc/mmc_host', locator)
        self.assertIn('sv08_emmc_device_at(SV08_EMMC_HOST_SYSFS', source)
        self.assertIn('#define SV08_EMMC_SECTORS 61079552ULL', locator)
        ...
        self.assertNotRegex(locator, r'\b(open|pread|pwrite|write|ioctl|system|popen)\s*\(')
        self.assertNotIn('O_RDWR', locator)
        self.assertIn('EMMC_LOCATOR,', builder)
    ...
    def test_controller_locator_ignores_mmc_host_number_and_rejects_ambiguity(self):
        with tempfile.TemporaryDirectory(prefix='sv08-emmc-locator-') as directory:
            ...
            result = subprocess.run([str(binary)], check=True, text=True,
                                    capture_output=True)
            self.assertIn('numbering/uniqueness/refusal checks passed', result.stdout)
