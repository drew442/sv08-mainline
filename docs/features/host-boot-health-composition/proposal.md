...
One implementer owns the new `runtime/sv08_boot_health.py`, its focused tests,
necessary narrow transaction admission injection, host staging/service ordering,
and affected boot documentation. Existing state, RAUC, admission and systemd
mechanisms remain authoritative; no custom bootloader or application scheduler
is introduced. Retire the coordinator when an upstream integration provides the
same boot-bound health and preservation contract. No new runtime dependency or
upstream source modification is proposed. Preserve immutable/writable operating
modes: ordinary writable boots cannot be rejected merely because immutable
RAUC image-confirmation checks are inapplicable; trial confirmation remains an
immutable image operation. Include that nontrial regression in hbh-09.

This is queued existing host functionality, not a new optional feature. It does
not activate the printer, install on hardware, or authorize a physical reboot.
H02/H07 run only through the shared queue after reviewed artifacts are ready.
All record checks are offline; physical/release gates remain in the host checklist.
