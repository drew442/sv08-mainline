...
| H03 — Move spare eMMC to writer and reinstall, only if needed | Establish that remote staging cannot perform the selected operation; identify spare/reader, review complete artifact/hash/layout and readback command; collect all ready changes first | The v4 spare is reinstalled; A failed during preparation and the next U-Boot pass selected recovery. A corrected root tree now rebuilds and validates the identity initramfs hook; compose and independently review a complete replacement image before requesting this move. Readback and safe eject precede reinstall; arm H02 logging before reconnection. | Preserve exact artifact hash/readback evidence. Never request one swap per lane; a materially different artifact or failed write can require another cycle |
