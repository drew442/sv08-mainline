...
| H10 — Read current spare-eMMC boot state before another eMMC boot | The owner reports that the eMMC was removed before installing the SD card, so the network boot cannot see it in the current configuration. A new network-root probe and SD U-Boot candidate keep MMC2 disabled in U-Boot while applying the pinned CB1 PC3 input/pull-down for Linux. See [candidate and limits](host-sd-network-emmc-probe-20260926.md). Independent review and exact SD image readback must pass first. | During one side-lying session with the printer fully off and USB serial disconnected: remove the disposable SD; write only that SD at Beelink's writer and verify full direct-I/O readback; reinstall only the spare eMMC in the printer (factory module remains stored) and reinstall the reviewed SD. Stand the printer upright and reconnect Ethernet. Arm receive-only serial capture before reconnecting serial/powering the host. One boot runs the static NFS init and powers down. If the eMMC is not uniquely found, any read fails, either CRC/layout is invalid, or boot output differs from the reviewed path, stop without another reboot. | H10 passes only on an explicit read-only result for the one 32 GB MMC beneath SMHC2, with both copies CRC-valid and policy values recorded. Keep raw evidence private; publish only device/capacity class, CRC status, flags, order and counters. This does not infer A health or authorize a counter change. If the installed spare remains invisible, use the accepted USB-reader method; the network test is impossible while eMMC is absent. |
