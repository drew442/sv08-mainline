...
| H12 — Commission writerless full-eMMC reimage — queued after offline prototype and review | The QEMU prototype passes full raw-image write/readback and refusal cases. A real candidate image, complete target map and exact eMMC identification method are independently reviewed. Record the factory module as preserved, confirm the spare's contents are expendable or exported, and review the recovery procedure. An independent high-consequence reviewer must approve the exact action immediately before any physical write. Arm receive-only serial capture before connecting USB serial, which powers the host. | Keep the spare installed and factory eMMC stored; do not use the USB writer or move media. Boot the already-reviewed SD/NFS source with its read-only image export, then permit one explicitly authorized one-shot job to write and fully verify the exact image. Capture target identity, progress, flush, full readback hash, GPT/partition checks and subsequent boot. No heater, motion or MCU operation. | Stop before writing on any source/target overlap, ambiguous target identity, capacity/hash/layout mismatch, unavailable or changed source, power/network loss, write/readback error, or unexpected boot-policy state. An uncertain result remains stopped for recovery; never retry automatically. This commissioning is a single supervised hardware validation, not proof that physical writes are safe under all failures. Keep the factory eMMC stored. |
