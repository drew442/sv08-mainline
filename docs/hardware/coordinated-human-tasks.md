...
| H03 — Move spare eMMC to writer and reinstall, only if needed | V5 is fully composed, independently byte-reviewed, and staged on Beelink with SHA-256 `09fb7efb7637e3b360bbb6c76e5b2b3cf7e0164a69082122b18a05ac567d34a1`; write/readback procedure and target checks are prepared. Move only the installed spare into the USB writer and connect it to Beelink; keep the factory eMMC stored. | V4's A boot failed during preparation and the next U-Boot pass selected recovery. V5 contains the corrected persistent-identity initramfs hook. After the owner connects the spare, identify the exact block device, verify its capacity/model and that it is unmounted, then write/read back v5 and safely eject before reinstall. Arm H02 capture before reconnecting the printer. | Preserve exact artifact hash/readback evidence. Never request one swap per lane; a materially different artifact or failed write can require another cycle |
...
independent offline byte review and its compressed artifact was staged on
Beelink with a verified SHA-256. It includes the corrected initramfs hook in
both A/B boot partitions. V5 has not been written or booted; H03 is now the
single human hardware action needed before write/readback commissioning.
