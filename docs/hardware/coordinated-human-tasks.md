...
| H12 — Commission writerless full-eMMC reimage — queued after offline prototype and review | The synthetic QEMU full write/readback and [single-use network-claim prototype](host-network-emmc-one-shot-qemu-20260926.md) and offline H616 target-admission improvement are complete. The [readiness audit](host-network-emmc-reimage-readiness-20260926.md) records the real v5 diagnostic candidate and map as commissioning-only inputs, the owner-accepted backup state, and the remaining gaps: no production one-shot writer, no current CID match, and no network-writer high-consequence review. The exact candidate image, live CID, target map, recovery procedure and physical action must be reviewed together. Arm receive-only serial capture before connecting USB serial, which powers the host. | Keep the spare installed and factory eMMC stored; do not use the USB writer or move media. When the writer and artifact are ready, boot the reviewed SD/NFS source with its read-only image export, compare the live eMMC CID, then permit one explicitly authorized one-shot job to write and fully verify the exact image. Capture target identity, progress, flush, full readback hash, GPT/partition checks and subsequent boot. No heater, motion or MCU operation. | Stop before writing on any source/target overlap, ambiguous target identity, capacity/hash/layout mismatch, unavailable or changed source, power/network loss, write/readback error, or unexpected boot-policy state. An uncertain result remains stopped for recovery; never retry automatically. This commissioning is a single supervised hardware validation, not proof that physical writes are safe under all failures. Keep the factory eMMC stored. |
