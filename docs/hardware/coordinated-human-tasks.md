...
| H10 — Read current spare-eMMC boot state before another eMMC boot | The candidate has independent PASS WITH CONDITIONS review; the disposable SU02G SD was written with the exact reviewed 192 MiB image and passed direct-I/O readback. See the [write receipt](host-sd-network-emmc-probe-card-write-20260926.md). Beelink NFSv3/TCP export is ready, read-only/root-squash and restricted to Beelink and the expected printer address. The owner reports the spare eMMC remains out of the printer; factory eMMC stays stored. | With the printer fully off and USB serial disconnected: reinstall only the spare eMMC and the written disposable SD. Stand the printer upright and reconnect Ethernet. Keep USB serial disconnected until the coordinator arms and confirms receive-only capture, since connecting it powers the host. Then allow one supervised boot; the NFS init reads the two boot records and powers down. If the eMMC is not uniquely found, a read fails, either CRC/layout is invalid, or output differs from the reviewed path, stop without another reboot. | H10 passes only on `READ_ONLY_VALID_PAIR` for the one expected 32 GB MMC beneath SMHC2, with both copies CRC-valid and policy values recorded. Keep raw evidence private; publish only device/capacity class, CRC status, flags, order and counters. This does not infer A health or authorize a counter change. If the installed spare remains invisible, use the accepted USB-reader method; network boot cannot inspect a physically absent eMMC. |
