...
| H10 — Read current spare-eMMC boot state before another eMMC boot | The successful SD/NFS diagnostic did not access eMMC; current redundant environment copies and A counter are unknown. The reviewed layout uses two 64 KiB copies at byte offsets 4 MiB and 8 MiB; prepare a read-only parser/receipt and identify the prior spare module by model/capacity and private CID evidence. | With the printer fully unpowered (disconnect USB serial), owner lays it on its side, removes only the installed spare eMMC, and connects that module to Beelink's writer. Keep the factory eMMC stored and the disposable SD out of the printer. Coordinator verifies target identity and reads/CRC-checks both environment copies without writing. After inspection and any separately reviewed decision, reinstall the same spare and stand the printer upright; do not boot until capture and boot policy are reviewed. | Preserve both raw copies, exact tool/config version, target identity and parsed policy privately; publish only sanitized values and CRC status. This is read-only evidence and cannot infer A health, authorize a counter change, or establish normal boot. Combine SD removal and eMMC removal/reinsertion in one side-lying session |
