...
| H09 — Correct and retry the supervised SD/NFS diagnostic — **passed 2026-09-26** | The first physical attempt selected the SD loader twice but stopped before Linux because `CONFIG_HASH_VERIFY` was missing; see the [first-boot record](host-sd-network-first-boot-20260925.md). The corrected 192 MiB image SHA `53cc0b2696add39dae24480167d79807e72a15f824fbc9025aaf20a9aa63d08a` has independent GO WITH CONDITIONS review and passed write/readback on the identified SU02G card. | The owner reinstalled the disposable SD with the printer fully powered off, stood the printer upright, and reconnected serial after receive-only capture was waiting. The single supervised retry completed and powered down as designed. | The [v2 result](host-sd-network-first-boot-20260926.md) records SD loader, Linux, wired DHCP, read-only NFS root and the exact `SV08_SD_NFS_PASS root_ro=1 data_tmpfs=1 dhcp_address=1` marker. Keep both boot traces separate. The eMMC A counter remains unknown; inspect it before any later boot that may select eMMC. HDMI output, normal host operation and printer functions remain separate work |

| H10 — Read current spare-eMMC boot state before another eMMC boot | The owner reports that the eMMC was removed before installing the SD card, so the network boot cannot see it in the current configuration. The reviewed network-root probe and SD U-Boot candidate keep MMC2 disabled in U-Boot while applying the pinned CB1 PC3 input/pull-down for Linux. See [candidate and limits](host-sd-network-emmc-probe-20260926.md). Independent review is PASS WITH CONDITIONS; the exact SD image still needs a full direct-I/O readback after writing. Beelink's NFSv3/TCP export is ready, read-only/root-squash and restricted to Beelink and the expected printer address. | One side-lying session: keep the printer fully off and USB serial disconnected; remove only the disposable SD and connect it to Beelink's USB reader. The coordinator writes the reviewed image and verifies full direct-I/O readback. Then reinstall only the spare eMMC and the reviewed SD in the printer (factory module remains stored), stand it upright and reconnect Ethernet. Arm receive-only serial capture before reconnecting serial/powering the host. One boot runs the static NFS init and powers down. If the eMMC is not uniquely found, any read fails, either CRC/layout is invalid, or boot output differs from the reviewed path, stop without another reboot. | H10 passes only on `READ_ONLY_VALID_PAIR` for the one expected 32 GB MMC beneath SMHC2, with both copies CRC-valid and policy values recorded. Keep raw evidence private; publish only device/capacity class, CRC status, flags, order and counters. This does not infer A health or authorize a counter change. If the installed spare remains invisible, use the accepted USB-reader method; the network test cannot inspect a physically absent eMMC. |
