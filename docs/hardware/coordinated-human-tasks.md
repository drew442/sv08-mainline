...
| H10 — Read current spare-eMMC boot state | The reviewed SU02G SD is written and its direct-I/O readback matched. The first supervised boot reached Linux/DHCP but failed before the probe because NFSv3 mountd was not registered; no eMMC result was produced. The corrected init SHA `1e7afa9aaf337ccffdb4c7431648f7aba934dcc600b04e11751743e345e1dc11` is served and verified by default-port NFSv3/TCP mount. mountd v3/TCP and `.141` ro/root-squash export are checked; exact source, manifest and independent retry review are in the [H10 records](host-sd-network-emmc-probe-review-20260926.md). The owner now confirms the spare eMMC is not installed; factory module remains stored. | With USB serial disconnected and all host power removed, install only the spare eMMC in the printer; keep the same written SU02G SD in place and leave the factory module stored. Keep serial disconnected after installation. Before the one reviewed retry, reconfirm mountd v3/TCP, `.141` ro/root-squash export and deployed init hash, and arm fresh receive-only capture. Only then reconnect USB serial once to start the single boot. | H10 is incomplete until a unique 32 GB MMC under exact controller `4020000` is read with both environment copies CRC/layout-valid. No boot with the eMMC absent. Stop on NFS refusal, unexpected boot, absent/ambiguous MMC, read failure or invalid records. If the measured non-removable MMC initialization failure repeats, stop network retries and use the USB-reader path. Counters remain unknown; no eMMC, boot-policy or MCU writes. Keep raw captures private. |
